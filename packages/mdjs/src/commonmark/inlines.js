...
const { ESCAPABLE } = common;
const ESCAPED_CHAR = `\\\\${ESCAPABLE}`;

const { ENTITY } = common;
const { reHtmlTag } = common;
...
const normalizeReference = function(string) {
  return string
...
const parseBackticks = function(block) {
...
// is a backslash.
const parseBackslash = function(block) {
...
const parseAutolink = function(block) {
  let m;
  let dest;
  let node;
...
const parseHtmlTag = function(block) {
...
const handleDelim = function(cc, block) {
...
const removeDelimiter = function(delim) {
...
const removeDelimitersBetween = function(bottom, top) {
  if (bottom.next !== top) {
    bottom.next = top;
    top.previous = bottom;
  }
...
const processEmphasis = function(stack_bottom) {
  let opener;
  let closer;
  let old_closer;
  let opener_inl;
...
// null if no match.
const parseLinkDestination = function() {
...
const parseOpenBracket = function(block) {
  const startpos = this.pos;
  this.pos += 1;
...
const parseBang = function(block) {
  const startpos = this.pos;
  this.pos += 1;
...
const parseCloseBracket = function(block) {
  let startpos;
  let is_image;
  let dest;
  let title;
...
const addBracket = function(node, index, image) {
...
const removeBracket = function() {
  this.brackets = this.brackets.previous;
};

// Attempt to parse an entity.
const parseEntity = function(block) {
  let m;
...
const parseString = function(block) {
  let m;
  if ((m = this.match(reMain))) {
    if (this.options.smart) {
      block.appendChild(
        text(
          m.replace(reEllipses, '\u2026').replace(reDash, chars => {
            let enCount = 0;
            let emCount = 0;
            if (chars.length % 3 === 0) {
...
const parseNewline = function(block) {
...
const parseReference = function(s, refmap) {
  this.subject = s;
  this.pos = 0;
  let rawlabel;
  let dest;
...
const parseInline = function(block) {
  let res = false;
  const c = this.peek();
  if (c === -1) {
    return false;
...
const parseInlines = function(block) {
