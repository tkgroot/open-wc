...
const endsWithBlankLine = function(block) {
  while (block) {
...
const addChild = function(tag, offset) {
...
const parseListMarker = function(parser, container) {
...
const listsMatch = function(list_data, item_data) {
  return (
...
const closeUnmatchedBlocks = function() {
  if (!this.allClosed) {
...
const blocks = {
  ...
  html_block: {
    continue(parser, container) {
      return parser.blank && (container._htmlBlockType === 6 || container._htmlBlockType === 7)
        ? 1
        : 0;
    },
    finalize(parser, block) {
  ...
  function(parser, container) {
    ...
    if (
      ...
      while (
        ...
        (pos = parser.inlineParser.parseReference(container._string_content, parser.refmap))
      ) {
...
const advanceOffset = function(count, columns) {
...
const advanceNextNonspace = function() {
  this.offset = this.nextNonspace;
  this.column = this.nextNonspaceColumn;
  this.partiallyConsumedTab = false;
};

const findNextNonspace = function() {
  const { currentLine } = this;
  let i = this.offset;
...
const incorporateLine = function(ln) {
  ...
  if (!this.allClosed && !this.blank && this.tip.type === 'paragraph') {
    ...
    const lastLineBlank =
      ...
      !(
        ...
        (t === 'item' && !container._firstChild && container.sourcepos[0][0] === this.lineNumber)
      );
...
const finalize = function(block, lineNumber) {
...
const processInlines = function(block) {
  let node;
  let event;
  let t;
